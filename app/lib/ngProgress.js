...
module.provider('progressbar', function () {
    ...
    this.$get = ['$document', '$window', '$rootScope', function ($document, $window, $rootScope) {
        var count = this.count,
            height = this.height,
            color = this.color,
            $body = $document.find('body'),
        // Create elements that is needed
            progressbarContainer = angular.element('<div mb-progressbar class="progressbar-container"></div>'),
        ...
        progressbarContainer.css('position', 'absolute');
        progressbarContainer.css('margin', '0');
        progressbarContainer.css('padding', '0');
        progressbarContainer.css('top', '56px');
        ...
        return {
            ...
            message: function (message, color) {
                ...
                this.set('99.9%');
                setTimeout(function () {
                    progressbar.css('height', '50px');
                    progressbar.css('line-height', '50px');
                }, 500);
                setTimeout(function () {
                    progressbar.html('<div class="container">' + message + '<button class="right progressbar-close-message">X</button></div>');
                }, 1000);
            }
        };
    }];
